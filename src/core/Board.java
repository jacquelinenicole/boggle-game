...
public class Board implements IBoard  {
    ...
    public void populateDice() {
        ...
        for (int outerCounter = 0 ; outerCounter < NUMBER_OF_DICE ; outerCounter++) {
            ...
            for (int innerCounter = 0 ; innerCounter < NUMBER_OF_SIDES ; innerCounter++) {
                String s = diceData.get(outerCounter * NUMBER_OF_SIDES + innerCounter); //placeholder variable so each letter is added in order
    ...
    public void displayBoard() {
        ...
        for (String character: gameData) { //enhanced for loop to traverse array gameData
            System.out.print(character + " ");
           
            ++counter;
            if (counter%4 == 0) //every four characters, newline is printed
                System.out.print("\n");
        }  
    }
}