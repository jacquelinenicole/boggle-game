...
public class BoggleUi {
    ...
    private JMenu menu;
    private JMenuItem newGameMenuItem, exitMenuItem;
    private JPanel bottomPanel, buttonPanel, rightPanel;
    private JButton[][] buttonGrid;
    private JScrollPane scrollPane;
    private JTextPane textPane;
    private JLabel currentWordLabel, scoreLabel, timeLeftLabel;
    private JButton submitWordButton, shakeDiceButton;
    ...
    //custom constructor
    public BoggleUi(Board passedBoard, ArrayList<String> passedDictionary) {
        board = passedBoard;
    ...
    //allows strikethroughs for the textPane
    ...
    //initializes and sets all of the pane's components
    public void initComponents() {
        //sets up the frame
        frame = new JFrame();
        frame.setTitle("Boggle");
        frame.setVisible(true);
        frame.setPreferredSize(new Dimension(606, 459));
        frame.setMinimumSize(new Dimension(606, 459));
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        
        document = new StyleDocument();
        
        createMenu(); //initialize the JMenuBar and add to the JFrame 
        setupCurrentPanel(); //initialize the JPane for the current word
        setupWordPanel(); //initialize the JPanel for the word entry
        setupBogglePanel(); //initialize the JPanel for the Boggle dice
        
        //adds menu bar and panels to the frame
    ...
    //deals with menu bar options
    private void createMenu() {
        menuBar = new JMenuBar(); //initializes
        menu = new JMenu("Boggle");
        newGameMenuItem = new JMenuItem("New Game");
        exitMenuItem = new JMenuItem("Exit");
        menuBar.add(menu); //adds menu to menuBar
        menu.setMnemonic('b'); //underlines 'B' in "Boggle"
        menu.add(newGameMenuItem); //adds "New Game" option
        newGameMenuItem.setMnemonic('n'); //underlines 'N' in "New Game"
        newGameMenuItem.addActionListener(new MenuActionListener()); //registers action listener
        menu.add(exitMenuItem); //adds "Exit" option
        exitMenuItem.setMnemonic('e'); //underlines 'E' in "Exit"
        exitMenuItem.addActionListener(new ExitActionListener()); //registers action listener
    }
    ...
    private void setupCurrentPanel(){
        ...
        currentWordLabel.setPreferredSize(new Dimension(270, 50));
        
        //button to submit word
        bottomPanel.add(submitWordButton, ComponentOrientation.LEFT_TO_RIGHT); 
        submitWordButton.setPreferredSize(new Dimension(185, 50));
        submitWordButton.addActionListener(new SubmitWordActionListener());
        
        //label for player's score
        bottomPanel.add(scoreLabel, ComponentOrientation.LEFT_TO_RIGHT);
        scoreLabel.setPreferredSize(new Dimension(90, 50));
    ...
    private void setupWordPanel() {
        ...
        rightPanel.add(timeLeftLabel);
        timeLeftLabel.setHorizontalAlignment(SwingConstants.CENTER);
        timeLeftLabel.setPreferredSize(new Dimension(240, 90));
        timeLeftLabel.setMinimumSize(new Dimension(240, 90));
        timeLeftLabel.setMaximumSize(new Dimension(240, 90));
        timeLeftLabel.setBorder(BorderFactory.createTitledBorder("Time Left"));
        timeLeftLabel.setFont(new Font(timeLeftLabel.getName(), Font.PLAIN, 48)); 
        
        //button for shaking dice
        rightPanel.add(shakeDiceButton);
        shakeDiceButton.setPreferredSize(new Dimension(250, 90));
        shakeDiceButton.setMinimumSize(new Dimension(250, 90));
        shakeDiceButton.setMaximumSize(new Dimension(250, 90));
    }
    ...
    private void compareScores()
    {
        ...
        {
            JOptionPane.showMessageDialog(null, "You found 0 words");
        ...
        JOptionPane.showMessageDialog(null, "The computer found " + numComputerWords + " of Player's " + totalWords + " unique word(s).");
